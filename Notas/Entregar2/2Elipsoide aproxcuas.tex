...
Dado que se quiere resolver el problema del esparcimiento de luz por un elipsoide centrado en el origen, lo primero que hay que hacer, dado que se considera el límite cuasiestático, es resolver la ecuación de Laplace para el potencial; por lo cual, lo más conveniente es emplear las coordenadas anteriores. Para ello, se expresa al laplaciano en coordenadas generalizadas \cite{Arfken}
\begin{equation}
...
\begin{align}
    ...
    \label{phi0 con F},
\end{align}
 tiene que satisfacer la Ec. (\ref{laplaceplisoidal}). Analizando cada componente por separado
\begin{align}
...
\begin{wrapfigure}{r}[0pt]{0pt}
	...
	{\begin{tabular}{c}
			\begin{subfloatrow}
				\ffigbox[\FBwidth]
				{\includegraphics[width=.3\textwidth]{../../Figuras/prolato}}
				{\caption{Esferoide prolato}\label{esf_prolato}}
			\end{subfloatrow}\\
			
			\begin{subfloatrow}
				\ffigbox[\FBwidth]
				{\includegraphics[width=.3\textwidth]{../../Figuras/oblato}}
				{\caption{Esferoide oblato}\label{esf_oblato}}
			\end{subfloatrow}      
			{\caption{Clases especiales de elipsoides. El eje mayor del elipsoide está orientado en la dirección del eje a) $\hat{e}_z$ y b) $\hat{e}_y$.} \label{fig:ex}}%
	\end{tabular}}
\end{wrapfigure}
\begin{equation*}
